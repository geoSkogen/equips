...
class Schema_Ranker {
  ...
  public static function highest_in_arr($arr) {
    $highest = -1;
  ...
  public static function test_for_partial_match($long_arr,$short_arr) {
 ...
 //($stripped_query, $num_str, get_option('equips_images'), 'img')
  public static function test_for_best_match ($stripped_query, $num_str, $field_options, $field_type) {
      ...
      $count = !empty($field_options[$field_key]) ? $field_options[$field_key] : 0;
...
*/
              ...
              foreach ($raw_schema as $raw_scheme) {
                ...
                if ($clean_scheme === $match_string) {
                  ...
                  $match_score += ( count($this_query) >= count($match_arr) ) ?
                     self::test_for_partial_match($this_query, $match_arr) :
                     self::test_for_partial_match($match_arr, $this_query);
                }//end
      ...
      $top_score = self::highest_in_arr($scheme_scores);
      error_log("top score");
      error_log($top_score);
